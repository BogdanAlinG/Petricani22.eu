...
INSERT INTO amenities (category_id, slug, name_en, name_ro, icon, display_order) VALUES
  ((SELECT id FROM amenity_categories WHERE slug = 'bedroom'), 'air-conditioning', 'Air Conditioning', 'Aer Condiționat', 'Wind', 1),
  ((SELECT id FROM amenity_categories WHERE slug = 'bedroom'), 'heating', 'Heating', 'Încălzire', 'Flame', 2),
  ...
  ((SELECT id FROM amenity_categories WHERE slug = 'bathroom'), 'shower', 'Shower', 'Duș', 'Droplets', 1),
  ((SELECT id FROM amenity_categories WHERE slug = 'bathroom'), 'bathtub', 'Bathtub', 'Cadă', 'Bath', 2),
  ((SELECT id FROM amenity_categories WHERE slug = 'bathroom'), 'hairdryer', 'Hair Dryer', 'Uscător de Păr', 'Wind', 3),
  ((SELECT id FROM amenity_categories WHERE slug = 'bathroom'), 'toiletries', 'Toiletries', 'Articole de Toaletă', 'Sparkles', 4),
  ((SELECT id FROM amenity_categories WHERE slug = 'bathroom'), 'towels', 'Fresh Towels', 'Prosoape Curate', 'Layers', 5),
  ((SELECT id FROM amenity_categories WHERE slug = 'kitchen'), 'full-kitchen', 'Full Kitchen', 'Bucătărie Completă', 'ChefHat', 1),
  ...
  ((SELECT id FROM amenity_categories WHERE slug = 'kitchen'), 'dishwasher', 'Dishwasher', 'Mașină de Spălat Vase', 'Waves', 5),
  ((SELECT id FROM amenity_categories WHERE slug = 'entertainment'), 'smart-tv', 'Smart TV', 'Smart TV', 'Tv', 1),
  ((SELECT id FROM amenity_categories WHERE slug = 'entertainment'), 'wifi', 'High-Speed WiFi', 'WiFi de Mare Viteză', 'Wifi', 2),
  ((SELECT id FROM amenity_categories WHERE slug = 'entertainment'), 'streaming', 'Streaming Services', 'Servicii de Streaming', 'Play', 3),
  ((SELECT id FROM amenity_categories WHERE slug = 'entertainment'), 'bluetooth-speaker', 'Bluetooth Speaker', 'Boxă Bluetooth', 'Speaker', 4),
  ((SELECT id FROM amenity_categories WHERE slug = 'outdoor'), 'balcony', 'Balcony', 'Balcon', 'Home', 1),
  ((SELECT id FROM amenity_categories WHERE slug = 'outdoor'), 'terrace', 'Terrace', 'Terasă', 'TreePine', 2),
  ((SELECT id FROM amenity_categories WHERE slug = 'outdoor'), 'garden', 'Garden Access', 'Acces la Grădină', 'Flower2', 3),
  ((SELECT id FROM amenity_categories WHERE slug = 'outdoor'), 'bbq', 'BBQ Grill', 'Grătar', 'Flame', 4),
  ((SELECT id FROM amenity_categories WHERE slug = 'parking'), 'free-parking', 'Free Parking', 'Parcare Gratuită', 'Car', 1),
  ((SELECT id FROM amenity_categories WHERE slug = 'parking'), 'garage', 'Garage', 'Garaj', 'Warehouse', 2),
  ((SELECT id FROM amenity_categories WHERE slug = 'parking'), 'ev-charging', 'EV Charging', 'Încărcare Vehicule Electrice', 'Zap', 3),
  ...
  ((SELECT id FROM amenity_categories WHERE slug = 'safety'), 'first-aid', 'First Aid Kit', 'Trusă de Prim Ajutor', 'Cross', 3),
  ((SELECT id FROM amenity_categories WHERE slug = 'safety'), 'safe', 'Safe', 'Seif', 'Lock', 4),
  ((SELECT id FROM amenity_categories WHERE slug = 'services'), 'daily-cleaning', 'Daily Cleaning', 'Curățenie Zilnică', 'Sparkles', 1),
  ((SELECT id FROM amenity_categories WHERE slug = 'services'), 'laundry', 'Laundry Service', 'Serviciu de Spălătorie', 'Shirt', 2),
...
INSERT INTO house_rules (slug, title_en, title_ro, description_en, description_ro, icon, display_order) VALUES
  ('no-smoking', 'No Smoking', 'Fumatul Interzis', 'Smoking is not permitted anywhere on the property', 'Fumatul nu este permis nicăieri în proprietate', 'Ban', 1),
  ('no-parties', 'No Parties', 'Fără Petreceri', 'Parties and events are not allowed', 'Petrecerile și evenimentele nu sunt permise', 'PartyPopper', 2),
  ('pets', 'Pets Allowed', 'Animale Permise', 'Well-behaved pets are welcome with prior approval', 'Animalele de companie bine crescute sunt binevenite cu aprobare prealabilă', 'PawPrint', 3),
  ('quiet-hours', 'Quiet Hours', 'Ore de Liniște', 'Please maintain quiet between 10 PM and 8 AM', 'Vă rugăm să păstrați liniștea între 22:00 și 8:00', 'Moon', 4),
  ...
  ('check-out', 'Check-out Time', 'Ora de Check-out', 'Check-out must be completed by 11:00 AM', 'Check-out-ul trebuie finalizat până la 11:00', 'LogOut', 6)
ON CONFLICT (slug) DO NOTHING;
